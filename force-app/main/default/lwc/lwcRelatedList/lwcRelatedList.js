...
import getRecords from '@salesforce/apex/DTBL_RelatedList.getRecords';
import onSearch from '@salesforce/apex/DTBL_RelatedList.onSearch';
import buildFieldJSON from '@salesforce/apex/DTBL_RelatedList.buildFieldJSON';
...
const recordUpdatedSuccessMessage = 'Record updated!';
const recordDeletedSuccessMessage = 'Record deleted!';
...
export default class LwcDatatable extends NavigationMixin(LightningElement) {
    ...
    @api orderBy;
    @api rowActions;
    @api elevatedContext = false;
    @api
    get selectedRowIds() {
        return this._selectedRowIds;
    }

    set selectedRowIds(selectedRowIds = []) {
        this._selectedRowIds = [...selectedRowIds];
    }
    // Private Property
    ...
    connectedCallback() {
        ...
        if (this.rowActions) {
            this.rowActions = JSON.parse(this.rowActions);
        }
    ...
    init() {
        buildFieldJSON({
            ...
            elevatedContext: this.elevatedContext
        })
            .then((data) => {
                if (data) {
                    ...
                    console.log(records, cols, count);
                    this.colsJson = cols;
                    const colAc = Object.values(cols);
                    console.log('colAc: ', JSON.parse(JSON.stringify(colAc)));
    ...
    fetchRecords() {
        console.log('fetchRecords ', this.elevatedContext);
        getRecords({ soql: this.soql, elevatedContext: this.elevatedContext })
            .then((data) => {
                if (data) {
    ...
    handleRowAction(event) {
        ...
        console.log('handleRowAction', actionName, row);
        switch (actionName) {
            case 'edit':
                this.editRecord(row);
                break;
            ...
            case 'show_details':
                ...
                break;
            case 'navigate': {
                const pageReference = (this.rowActions || []).find(
                    (action) => action.name === 'navigate'
                )?.pageReference;
                const newPageReference = this.replacePageReferenceVariables(
                    pageReference,
                    row
                );
                this.navigate(newPageReference);
                break;
            }
            default:
        }
    ...
    handleButtonAction(event) {
        ...
        // find the button by index and check for a page reference
        const buttonIndex = event.target.dataset.index;
        const pageReference = (this.actionButtons || [])[buttonIndex]
            ?.pageReference;
    ...
    replacePageReferenceVariables(pageReference, record) {
        const newPageReference = { ...pageReference };
        // iterate through attributes and state properties and replace variables with record values
        Object.keys(newPageReference.attributes || {}).forEach((key) => {
            newPageReference.attributes[key] = this.replaceVariables(
                newPageReference.attributes[key],
                record
            );
        });
        Object.keys(newPageReference.state || {}).forEach((key) => {
            newPageReference.state[key] = this.replaceVariables(
                newPageReference.state[key],
                record
            );
        });
    ...
    replaceVariables(value, record) {
        // check if string contains variable in {{variableName}} format
        if (value && value.includes('{{') && value.includes('}}')) {
            // replace variable with the same value from the record
    ...
    navigate(pageReference) {
        if (!pageReference) {
            this.showToast(
                'Error',
                'No page reference found for the action.',
                'error'
            );
            return;
        }
    ...
    handleRowSelection(event) {
        ...
        this._selectedRowIds = this.selectedRows.map((row) => row.Id);
        console.log(JSON.parse(JSON.stringify(this.selectedRows)));
        console.log(JSON.parse(JSON.stringify(this._selectedRowIds)));
        this.dispatchEvent(
            new FlowAttributeChangeEvent('selectedRowIds', this._selectedRowIds)
        );
    }
